...
var ProjectList = function()
{
    ...
    */
    self.addPoint = function(map, row, location)
    {
        ...
        options["title"] = row.project_name;
        options["icon"] = image;
        ...
        '<h3>' + row.project_name + ', ' + row.state + '</h3>';
        ...
        contentString +=
        '<p>' + 
            'Prices From $' + row.prices_from + '+<br/>' +
        '</p>' +
...
var ProjectDetail = function()
{
    ...
    this.init = function()
    {
		...
		{
		$('.tabs > li:last-child').show();
        $('.tabNav li:last-child a').addClass('active');
...
{
    var objDetail = new ProjectDetail();
