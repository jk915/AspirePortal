...
var refeshMeta = function(){
    ...
    $.post(base_url + 'admin/areamanager/ajaxwork', parameters,function(data){
        if (data) {
...
$(function(){
    ...
    $("#frmArea #button").live("click",function(e){
	...
	$("#tabGallery").click(function()
	{
		...
		var gUploader = new qq.FileUploader(
		{
			...
			// path to server-side upload script
			action: base_url + 'admin/areamanager/upload_file/gallery_image',
		    params: {
                "area_id" : $("#area_id").val(),
                "doc_id" : false,
                "doc_name" : false
	...
	var hrUploader = new qq.FileUploader(
	{
		// pass the dom node (ex. $(selector)[0] for jQuery users)
			...
			// path to server-side upload script
			action: base_url + 'admin/areamanager/upload_file/hero_image',
		    params: {
		      "area_id" : $("#area_id").val(),
		      "doc_id" : false,
		      "doc_name" : false,
		    ...
		    onComplete: function(id, fileName, responseJSON)
		    {
    			if(responseJSON.success) {
                    ...
                    $('#area_hero_img').show();
                    $('.hero_img').html('<img id="area_hero_img" src="'+responseJSON.fileName+'" width="250"/>');
...
//    });
    ...
    $("#delete_hero_image").live("click",function(e){
        if (confirm('Are you sure you want to delete the image ?')) {
            ...
            $.post(base_url + 'admin/areamanager/ajaxwork',parameters, function(){
               $('#area_hero_img').remove();
    ...
    $(".download_area").live("click",function(e){
        ...
        $.download(base_url + 'admin/areamanager/ajaxwork',paremeters);
    ...
    $('#save_file_changes').live('click',function(){
        ...
        $.post(base_url + 'admin/areamanager/ajaxwork',params,function(rs){
...
function refreshGalleryFiles(selected_folder)
{
    ...
    blockElement('#files_listing');
    $("#page_listing").load(base_url + 'admin/areamanager/ajaxwork', parameters,function(){
       unblockElement('#files_listing');
    });
}