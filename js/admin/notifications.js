...
var refeshMeta = function(){
    ...
    $.post(base_url + 'admin/notifications/ajaxwork', parameters,function(data){
        if (data) {
...
$('.update_weekly_sales_report').live('click',function(){
        var parameters = {};
        var weekly_sales_report = $('.weekly_sales_report').val();
        
		parameters['type'] = 3;
        parameters['weekly_sales_report'] = weekly_sales_report;
        parameters['user_id'] = $('.update_weekly_sales_report').attr('uid');
        blockElement("#frm_weekly_sales_report");
        $.post(base_url + 'admin/notifications/ajaxwork', parameters,function(data){
            unblockElement("#frm_weekly_sales_report");
            if (data.success) {
				 
                $.fancybox.close();
				window.location.reload();
            } else {
                alert('Can not update user mail Notification.');
            }
        },'json');
    });

$('.update_new_listing_email').live('click',function(){
        var parameters = {};
        var weekly_sales_report = $('.weekly_sales_report').val();
        
		parameters['type'] = 4;
        parameters['weekly_sales_report'] = weekly_sales_report;
        parameters['user_id'] = $('.update_new_listing_email').attr('uid');
        blockElement("#frm_new_listing_email");
        $.post(base_url + 'admin/notifications/ajaxwork', parameters,function(data){
            unblockElement("#frm_new_listing_email");
            if (data.success) {
...
$(function(){
	    ...
	    $(".doc_upload_file").each(function(){
        	...
        	var dUploader = new qq.FileUploader(
        	{
        		// pass the dom node (ex. $(selector)[0] for jQuery users)
        			...
        			action: base_url + 'admin/notifications/upload_file/documents',
        		    params: {
	...
	$("#tabGallery").click(function()
	{
		...
		var gUploader = new qq.FileUploader(
		{
			...
			// path to server-side upload script
			action: base_url + 'admin/notifications/upload_file/gallery_image',
		    params: {
                "region_id" : $("#region_id").val(),
                "doc_id" : false,
                "doc_name" : false
	...
	var hrUploader = new qq.FileUploader(
	{
		// pass the dom node (ex. $(selector)[0] for jQuery users)
			...
			// path to server-side upload script
			action: base_url + 'admin/notifications/upload_file/hero_image',
		    params: {
...
//    });
    ...
    $("#delete_hero_image").live("click",function(e){
        if (confirm('Are you sure you want to delete the image ?')) {
            ...
            $.post(base_url + 'admin/notifications/ajaxwork',parameters, function(){
    ...
    $(".download_area").live("click",function(e){
        ...
        $.download(base_url + 'admin/notifications/ajaxwork',paremeters);
    ...
    $('#save_file_changes').live('click',function(){
        ...
        $.post(base_url + 'admin/notifications/ajaxwork',params,function(rs){
...
function refreshGalleryFiles(selected_folder)
{
    ...
    blockElement('#files_listing');
    $("#page_listing").load(base_url + 'admin/notifications/ajaxwork', parameters,function(){
       unblockElement('#files_listing');
    });
}
...
$("#submitorder").live("click",function(){
	{
		...
		if($('#type').val() == 'new_listing_email') {
			$.post(base_url + 'cron/new_listing_notification', parameters , function(data)
			{
			...
			$.post(base_url + 'cron/weekly_sales_notification', parameters , function(data)
			{
				alert('Mail Sent Successfully');
...
$("#update_notifications").live("click",function() {

    var parameters = {};
    parameters['type'] = 5;
    parameters['notification_id'] = $('#mail_type').val();
    parameters['turn_on_off'] = $('#turn_on_off').val();
    parameters['duration'] = $('#duration').val();
    parameters['due_date'] = $('#mail_date').val();
    
    $.post(base_url + 'admin/notifications/ajaxwork', parameters,function(data){
        if (data.success == '1') {
				window.location.reload();
            } else {
                alert('Can not update Mail Notification.');
            }
...
$('#mail_type').live('change',function() {
