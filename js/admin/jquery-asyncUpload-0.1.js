...
(function($) {
    $.fn.makeAsyncUploader = function(options) {
        return this.each(function() {
            ...
            var id = $(this).attr("id");    
            var displayUploader = "";
            var default_button_image_url  = options.button_image_url;
                        
            //-------for documents only!!!: check if the upload is visible-----
            if (id.indexOf("doc_") == 0 || $(this).hasClass("showif"))
            {
                var isVisible = true;                        
                ...
                if(!isVisible)
                {
                    displayUploader = "style='display:none'";
                }
                    
                    
                //change button image    
                if (id.indexOf("doc_") == 0)
                      options.button_image_url = base_url + 'images/admin/attachfile.png';    
            }            
            
            //------END documents --- 
                
            var container = $("<span id='async_"+id+"' class='asyncUploader' " + displayUploader + " />");
            container.append($("<div class='ProgressBar'> <div>&nbsp;</div> </div>"));
            container.append($("<span id='" + id + "_completedMessage'/>"));
            container.append($("<span id='" + id + "_uploading'>Uploading... <input type='button' class='smallbutton' value='Cancel'/></span>"));
            ...
            var defaults = {
                ...
                button_text: "<font face='Arial' size='13pt'>Save & Upload</span>",
                button_text_left_padding: (width - 90) / 2,
                button_text_top_padding: 1,
                
                // Called when the user chooses a new file from the file browser prompt (begins the upload)
                file_queued_handler: function(file) {
                     
                     var bStartUpload = false;
                     current_swfu_id = id;
                     
                     if (options.beforeUpload)
                     {
                         bStartUpload = eval(options.beforeUpload);
                     }
                     else
                     {  
                         bStartUpload = beforeUploadStart();
                     }
              
                     if (bStartUpload) { swfu_array[id].startUpload(); }
                ...
                upload_start_handler: function() {
                    swfu_array[id].setButtonDimensions(0, height);
                ...
                upload_complete_handler: function() {
                    var clearup = function() {
                        ...
                        $("span[id$=_uploading]", container).hide();
                        swfu_array[id].setButtonDimensions(width, height);
                    };
                ...
                upload_complete_handler : function(event, file){
                    ...
                    var message_input = this.settings.button_placeholder_id.replace("swf","guid")
                    var message = $('input[name="'+message_input+'"]').val();
                                                                
                    if (message && message.indexOf("Error")<0)
                    {
                        //swfu.startUpload();
                        swfu_array[id].startUpload();
                    }
                    else
                        alert(message);
            ...
            options.button_image_url = default_button_image_url;            
            ...
            $("span[id$=_uploading] input[type='button']", container).click(function() {
                swfu_array[id].cancelUpload(null, false);
            });
