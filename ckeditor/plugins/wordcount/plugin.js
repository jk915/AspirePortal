        ...
        if ($('div#cke_wordcount_'+editor.name).length > 0) { // Check element exists
            ...
            setTimeout(function() {
                ...
                if (editor.config.wordcount_maxWords > 0) {
                    // Display with limit
                    $('div#cke_wordcount_'+editor.name).html('Character Count: '+wordCount+'/'+editor.config.wordcount_maxWords);
                } else {
                    // Just display word count
                    $('div#cke_wordcount_'+editor.name).html('Character Count: '+wordCount); 
                }
    ...
    CKEDITOR.plugins.add('wordcount', {
        init: function(editor) {
            ...
            setTimeout(function() {
                if (editor.config.wordcount_maxWords > 0) { 
                    // Display with limit
                    $('td#cke_bottom_'+editor.name).append('<div id="cke_wordcount_'+editor.name+'" style="display: inline-block; float: right; text-align: right; margin-top: 5px; cursor:auto; font:12px Arial,Helvetica,Tahoma,Verdana,Sans-Serif; height:auto; padding:0; text-align:left; text-decoration:none; vertical-align:baseline; white-space:nowrap; width:auto;">Character Count: '+GetWordCount(editor.getData())+'/'+editor.config.wordcount_maxWords+'</div>');
                } else {
                    // Just display word count
                    $('td#cke_bottom_'+editor.name).append('<div id="cke_wordcount_'+editor.name+'" style="display: inline-block; float: right; text-align: right; margin-top: 5px; cursor:auto; font:12px Arial,Helvetica,Tahoma,Verdana,Sans-Serif; height:auto; padding:0; text-align:left; text-decoration:none; vertical-align:baseline; white-space:nowrap; width:auto;">Character Count: '+GetWordCount(editor.getData())+'</div>');
                }
            }, 4000);
