<body id="contact">
    <div id="wrapper">
        ...
        <div id="content">
            ...
            <?php $this->load->view("admin/region/navigation"); ?>                        
            
            <form class="plain" id="frmRegion" name="frmRegion" action="<?php echo base_url()?>admin/regionmanager/region/<?php echo $region_id?>"  method="post">
                <input type="hidden" id="folder" value="<?php echo $region_id;?>" />
                <input type="hidden" id="region_id" value="<?php echo $region_id;?>" />
     
<?php if(isset($region)) : // We're editing and existing region.  Show the tabs. ?>
                
				<br><br>
			    <input id="submitbutton2" class="button right" type="button" value="<? echo ($region_id == "") ? "Create New Region": "Update Region"; ?>" style="margin-top:-40px;" />
...
<?php endif; ?>
						...
						<div class="left" style="padding-top: 20px;">
							<label for="region_name">Region Name:<span class="requiredindicator">*</span></label>
                    		<input id="region_name" class="required" type="text" value="<? echo ($region_id !="") ? $region->region_name : ""; ?>" name="region_name"/>						
                            ...
                            <select id="state_id" name="state_id" class="required">
                                ...
                                <?php 
                                if($region)
                                    echo $this->utilities->print_select_options($states,"state_id","preferredName",($region_id !="") ? $region->state_id : ""); 
                                ?>
                            </select>   
							<br/><br/>
							<input type="checkbox" name="enabled" value="1" class="left" <? echo ($region_id !="") ? (($region->enabled == 1) ? "checked" :"") : "checked" ?>  /><label for="enabled" class="left" style="padding-top:0px">&nbsp;Enabled</label> 
    					</div>		
                           
                            
<?php if(isset($region)) : ?>
						<div class="left" style="width:33%">	
						<label for="region_hero_image">region Hero Image</label>
                            <div class="hero_img">
                                <?php if (!empty($region->region_hero_image)) : ?>
                              	<img id="region_hero_img" src="<?php  echo base_url().$region->region_hero_image; ?>" width="250" class="<?php echo ($region->region_hero_image == "") ?  "hidden" : ""; ?>" />
                              	<?php endif; ?>
                          	</div>
        					<input class="<?php echo ($region->region_hero_image == "") ?  "hidden" : ""; ?> button" type="button" value="Delete Hero Image" id="delete_hero_image" />
        					<!--<input type="file" name="region_hero_image" id="region_hero_image" class="showif <?php echo (!empty($region->region_hero_image)) ?  "hidden" : ""; ?>" />  -->
        					<div id="hero_image_upload" class="showif <?php echo (!empty($region->region_hero_image)) ?  "hidden" : ""; ?>"></div>
                        </div>
<?php endif; ?>
                        <div style="width:33%; float:left;">
						
						<?php if(isset($region)) : ?>
                            <p>Last Modified: <?=$this->utilities->isodatetime_to_ukdate($region->last_modified); ?></p>
                            <?php endif; ?> 
						</div>
...
<?php if(isset($region)) : ?>
                    ...
                    <div>
                        ...
                        <textarea id="wysiwyg" cols="20" rows="10" name="short_description" style="width:880px;height:300px" class="editor"><? echo ($region_id !="") ? $region->short_description : "" ?></textarea>
                        <input type="hidden" name="wysiwygWordCount" value="310" />
                        
                        <div class="clear"></div>
                        
                        <label for="overview">Overview</label><br />
                        <textarea id="wysiwyg2" cols="20" rows="10" name="overview" style="width:880px;height:300px" class="editor"><? echo ($region_id !="") ? $region->overview : "" ?></textarea>
                        <input type="hidden" name="wysiwyg2WordCount" value="790" />
                    ...
                    <div>
                        ...
                        <div id="files_listing">
                            ...
                            <div  id="page_listing">
                                <? $this->load->view('admin/region/file_listing',array('files'=>$images,'pages_no' => count($images) / $images_records_per_page)); ?>
                            </div>
...
<?php endif; ?>
                ...
                <input id="button" type="submit" value="<? echo ($region_id == "") ? "Create New Region": "Update Region"; ?>" /><br/>                    
            </form>
