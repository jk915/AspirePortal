   ...
   <script type="text/javascript" src="<?php echo base_url(); ?>js/admin/page.js"></script>
   ...
   {
		// If the user has just tabbed out of the page_title field,
		// see if the page_code field is blank.   If it is, create a page code
		// for the user automagically.
		$("#page_title").bind('blur',function()
		{
			// Make sure the current code is blank
			var current_code = $("#page_code").val();

			if(current_code != "")
				return;

			// Get the article title
            var code = $("#page_title").val();
            if(code == "")
            	return;	
			...
			code = code.replace(/[^a-z-]/g, "");	// Replace other punctionation with nothing   
