                ...
                <div class="content">
					...
					<div class="sidebar">
                        ...
                        <?php if(in_array($utid, array(USER_TYPE_ADVISOR))) : ?>
                            ...
                            <p>
                                <label><input type="checkbox" value="25" class="view_all_properties" <?php echo $user->view_all_property == 1 ? 'checked="checked"' : ''?>> Lead can view ALL properties.</label>
                            </p>
                        ...
                        <?php if(in_array($utid, array(USER_TYPE_PARTNER))) : ?>
                            <h3>Stock Access</h3>
                            <?php
                                if($user->view_all_property == 1):
                            ?>
                            <p>
                                This member has access to all properties
                            </p>
                            <?php else: ?>
                            <div class="stock_permissions">
                                <div class="stock_property_permissions"></div>
                        ...
                        <ul>
                            ...
                            <?php endif; ?>
                            <?php if((substr($user->username, 0, 5) == "LEAD_") && (in_array($utid, array(USER_TYPE_ADVISOR)))) : ?>
                            <li><a href="javascript:;" data-reveal-id="allowLeadLogin" class="allow_lead_login">Allow this Enquiry to login</a></li> 
                            <?php else: ?>
                            	<?php if (($user->enabled == 1) && (in_array($utid, array(USER_TYPE_ADVISOR)))) : ?>
                    ...
                    <div class="mainCol"> 
                        <?php echo form_open('leads/ajax', array("id" => "frmLeadDetail", "name" => "frmLeadDetail", "class" => "block")); ?>
                            ...
                            <fieldset>                            
                                ...
                                <input type="text" name="middle_name" value="<?php echoifobj($user, "middle_name"); ?>" id="middle_name" />                                
                                ...
                                <input type="text" name="secondary_email" value="<?php echoifobj($user, "secondary_email"); ?>" id="email" class="secondary_email"  />                                
                            ...
                            <fieldset>
                                ...
                                <input type="text" name="delivery_address1" value="<?php echoifobj($user, "delivery_address1"); ?>" id="delivery_address1" />
                                
                                <!--
                                <label for="delivery_address2">Address Line 2 SLT</label>
                                <input type="text" name="delivery_address2" value="<?php echoifobj($user, "delivery_address2"); ?>" id="delivery_address2" />
                                -->
                                
                                <label for="delivery_suburb">Suburb</label>
                                <input type="text" name="delivery_suburb" value="<?php echoifobj($user, "delivery_suburb"); ?>" id="delivery_suburb" />
                                
                                <label for="delivery_postcode">Postcode</label>
                                <input type="text" name="delivery_postcode" value="<?php echoifobj($user, "delivery_postcode"); ?>" id="delivery_postcode" />            
                                
                                <label for="delivery_state_id">State</label>
                                <select name="delivery_state_id" id="delivery_state_id">
                                    <option value="">Choose</option>
                                    <?php echo $this->utilities->print_select_options($states, "state_id", "name", ($user) ? $user->delivery_state_id : ""); ?>
                                </select>   
                                
                                <!-- hard coded country id for now -->
                                <input type="hidden" name="delivery_country_id" value="1" />                             
                            </fieldset>
                            
                            <div class="clear"></div>
                            
                            <label for="comment">Comments</label>
                            <textarea id="comment" name="comment" cols="30" rows="6" class="fullwidth"><?php echoifobj($user, "comment"); ?></textarea>
                            ...
                            <fieldset>
                                ...
                                <?php if($utid != USER_TYPE_PARTNER) : ?>
                                <label for="advisor_id">Additional Advisor</label>
                                ...
                                <label for="owner_id">Partner</label>
                                <?php if((!$user) || ($user->created_by_user_id == $user_id) || ($utid == USER_TYPE_ADVISOR)) : ?>
                            ...
                            <fieldset>
                               <?php if($user) : ?>
                                    <?php if(in_array($utid, array(USER_TYPE_ADVISOR)) && substr($user->username, 0, 5) != "LEAD_") : ?>
                                    <label for="enabled" class="top-margin20">
                                ...
                                <label for="other_lead_source" class="top-margin20">Other Lead Source</label>
                                <?php if((!$user) || ($user->created_by_user_id == $user_id)) : ?>
                            ...
                            <textarea id="keywords" name="keywords" cols="5" rows="6" class="textwidth" height="50px"><?php echoifobj($user, "keywords"); ?></textarea>		
                        	...
                        	<fieldset>                                         
                                ...
                                <input type="text" name="additional_contact_first_name" value="<?php echoifobj($user, "additional_contact_first_name"); ?>" id="additional_contact_first_name" />
                                
                                <label for="additional_contact_middle_name">Middle Name</label>
                                <input type="text" name="additional_contact_middle_name" value="<?php echoifobj($user, "additional_contact_middle_name"); ?>" id="additional_contact_middle_name" />
                                
                                <label for="additional_contact_middle_name">Last Name</label>
                                <input type="text" name="additional_contact_last_name" value="<?php echoifobj($user, "additional_contact_last_name"); ?>" id="additional_contact_last_name" />                                
                                                                                    
                                <label for="additional_contact_relationships">Relationship</label>
                                <select name="additional_contact_relationships">
                                    <option value="">Choose</option>
                                    <?php echo $this->utilities->print_select_options_array($relationship_types, false, ($user) ? $user->additional_contact_relationships : '');?>
                                </select>                                
                        	</fieldset>
                        	<fieldset>
                                ...
                                <input type="text" name="additional_contact_email" value="<?php echoifobj($user, "additional_contact_email"); ?>" id="additional_contact_email" />                                
                                
                                <label for="additional_contact_comment">Comment</label>
                                <input type="text" name="additional_contact_comment" value="<?php echoifobj($user, "additional_contact_comment"); ?>" id="additional_contact_comment" />                                                                
                        	</fieldset>
                            <div class="clear"></div>
                            <h3>Legal Details</h3>
                            
                            <label for="legal_purchase_entity">Full Legal Purchase Entity</label>
                            <textarea id="legal_purchase_entity" name="legal_purchase_entity" cols="30" rows="6" class="fullwidth"><?php echoifobj($user, "legal_purchase_entity"); ?></textarea>                            
                            
                            <label for="purchase_comments" class="top-margin20">Purchase comments and ownership split</label>
                            <textarea id="purchase_comments" name="purchase_comments" cols="30" rows="6" class="fullwidth"><?php echoifobj($user, "purchase_comments"); ?></textarea>                                                        
                            
                            <div class="top-margin20"></div>
                            
                            <fieldset>
                                <label for="acn">ACN</label>
                                <input type="text" name="acn" value="<?php echoifobj($user, "acn"); ?>" id="acn" />
                            </fieldset>                            
                            
                            <fieldset>
                                <label for="smsf_purchase">SMSF Purchase</label>
                                <input type="radio" name="smsf_purchase" value="Yes" <?php checkedif($user, "smsf_purchase", "Yes"); ?> /> Yes &nbsp; &nbsp;
                                <input type="radio" name="smsf_purchase" value="No" <?php checkedif($user, "smsf_purchase", "No"); ?> /> No
                            </fieldset>
