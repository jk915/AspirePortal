...
class States extends MY_Controller 
{
    ...
    function index()
    {
        ...
        $this->data["region_states"] = $this->state_model->get_list(1, "", "", $count_all, "", "state_name ASC");
	
        $this->data["states"] = $this->db->order_by("name ASC")->where("country_id", 1)->get("states");
               
        $this->load->view('member/header', $this->data);
        $this->load->view('member/state/list/prebody.php', $this->data); 
		$this->load->view('member/state/list/main.php', $this->data);
        $this->load->view('member/footer', $this->data); 
    
	}
    
    function detail($state_id = "")
    {
        ...
        $region_state = $this->state_model->get_details($state_id);
        ...
        $this->data["region_state"] = $region_state;
        ...
        $this->data["meta_title"] = "State " . $region_state->state_name;
        
        $this->load->model('Comment_model');
        //$this->data['comments'] = $this->Comment_model->get_list(array('foreign_id'=>$area_id,'type'=>'area_comment'));
        
        $this->load->view('member/header', $this->data);
        $this->load->view('member/state/detail/prebody.php', $this->data); 
        $this->load->view('member/state/detail/main.php', $this->data);
        $this->load->view('member/footer', $this->data); 
    }
    ...
    function ajax()
    {
        ...
        switch($action)
        {
            case "load_statelist":   // User is trying to update their own account
                $this->handle_load_statelist();
                break;
	...
	private function handle_load_statelist()
    {
		...
		$region_states = $this->state_model->get_list(1, "", "", $count_all, "", "state_name ASC");

        switch($list_type)
        {
            case "list":
                $this->data["message"] = $this->load->view("member/state/list/list", array("region_states" => $region_states), true);
                break;
                
            case "grid":
                $this->data["message"] = $this->load->view("member/state/list/grid", array("region_states" => $region_states), true);
                break;
                
            case "map":
                // For the map view, just send back the raw state data
                $array = array();

                if($region_states)
                {
                    foreach($region_states->result() as $region_state)
                    {
                        $item = array();
                        //$item["project_id"] = $project->project_id;
                        
                        $item["area_name"] = $region_state->state_name;
                        $item["state"] = $region_state->state_name;
                        $item["median_house_price"] = $region_state->median_house_price;
                        $item["rate"] = 'medium';
                        $item["url"] = base_url() . "state/detail/" . $region_state->state_id;
                        $item["lat"] = "";
                        $item["lng"] = "";
                        
                        // Get the lat/lng pairs out of the embed url
                        $found = preg_match_all("/ll=[-\d\.]*,[-\d\.]*/", $region_state->googlemap, $matches);
                        ...
                        if($region_state->state_hero_image != "")
                        {
