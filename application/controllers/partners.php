...
class Partners extends MY_Controller 
{
    ...
    function index()
    {
        $this->data["meta_title"] = "My Partners";
		
		$this->data["user_type_id"] = $this->user_type_id;
        
        $this->load->view('member/header', $this->data);
        $this->load->view('member/partners/list/prebody.php', $this->data); 
        $this->load->view('member/partners/list/main.php', $this->data);
    ...
    function detail($user_id = "")
    {
        ...
        if($add_mode)
        {
            $this->data["meta_title"] = "Add New Partner";    
        }
        else
        {
            ...
            $this->data["stats"] = $this->Users_model->get_partner_stats($user_id);
            
            $this->data["meta_title"] = "Partner Detail";
        }
        ...
        $this->load->view('member/header', $this->data);
        $this->load->view('member/partners/detail/prebody.php', $this->data); 
        $this->load->view('member/partners/detail/main.php', $this->data);
        $this->load->view('member/footer', $this->data); 
    }    
    ...
    function ajax()
    {
        ...
        switch($action)
        {
            case "update_partner":   // User is trying to login.
                $this->handle_update_partner();
                break;
                
            case "load_partners":
                $this->handle_load_partners();
                break;
    ...
    */
    private function handle_load_partners()
    {
        ...
        $extra_sql = ", get_partner_status_count(u.user_id, 'sold') as num_sold, get_partner_last_status_date(u.user_id, 'sold') as last_sold_date ";
        ...
        $users = $this->Users_model->get_list(-1, $limit = PARTNERS_PER_PAGE, $page_no = $current_page, $count_all, "", $user_type = USER_TYPE_PARTNER, $filters, $extra_sql);        
        //$users['user_type_id']=$user_type_id;
        $this->data["status"] = "OK";
        $this->data["message"] = $this->load->view("member/partners/list/list", array("users" => $users,'user_type_id'=>$this->user_type_id,"USER_TYPE_ADVISOR"=>USER_TYPE_ADVISOR), true);
    ...
    * Send an OK status back if the user was updated/added successfully, error if not.
    */
    private function handle_update_partner()
    {
        // Load neccessary libs and models
        $this->load->library('form_validation');
        ...
        if($add_mode)
        {
            ...
            $save["user_type_id"] = USER_TYPE_PARTNER;
    ...
    */
    private function handle_load_stock_project_permissions()
    {
        ...
        if($user_id)
        {
	        ...
	        $filters_project["permission_type"] = 'Project';
	        $filters_project["user_id"] = $user_id;
	        
			$assign_projects = $this->ppmd->get_list($filters_project, 'created_dtm ASC', 0, 0);
            ...
            $this->data["status"] = "OK";
            $this->data["message"] = $this->load->view("member/partners/detail/stock_project_permissions", $data, true);
            send($this->data);
        }
        else
        {
    ...
    */
    private function handle_load_property_permissions()
    {
        ...
        if($user_id)
        {
            ...
            $filters_property = array();
	        $filters_property["permission_type"] = 'Property';
	        $filters_property["user_id"] = $user_id;
	        //$filters_property["project_id"] = !empty($project_id) ? $project_id : -1;
	        
			$assign_properties = $this->ppmd->get_list($filters_property, 'created_dtm ASC', 0, 0);
            ...
            $this->data["status"] = "OK";
            $this->data["message"] = $this->load->view("member/partners/detail/stock_property_permissions", $data, true);
            send($this->data);
        }
        else
        {
