...
class Contentmanager extends CI_Controller 
{
    ...
    */
	function add($category_id)
	{
        ...
        if(!$category) {
            show_error("Could not load category");    
        }

		$website_id = $this->utilities->get_session_website_id(); 

		//$this->data["categories"] = $this->article_category_model->get_list($category->parent_id);
        $this->data["categories"] = $this->article_model->get_article_categories(-1, 1);
        
		$this->data["websites"] = $this->website_model->get_list(array());
		$this->data["category_id"] = $category_id;
        $this->data["root_category"] = false;    
	...
	*/
	function article($article_id = "", $category_id = -1)
	{
        ...
        if((is_numeric($category_id)) && ($category_id > 0)) {
            $this->data["root_category"] = $this->article_category_model->find_root_category($category_id);    
        } else {
            $this->data["root_category"] = false;    
        }
	...
	function category($category_id = "")
	{
        ...
        if($category->parent_id > 0) {
            $this->data['parent_category'] = $this->article_category_model->get_details($category->parent_id);
	...
	function ajaxwork()
	{
      ...
      switch($type)
      {
         ...
         case 6: //refresh articles list
            ...
            $return_data["html"] = $this->load->view('admin/contentmanager/content_listing',array('articles' => $articles, 'category_id' => $category_id, 'category' => $category),true); 
