...
class Media extends MY_Controller 
{
    ...
    function index()
    {
        $this->data["meta_title"] = "Media & Research";
        
        // Load available document types
        $this->data["document_types"] = $this->db->order_by("name", "ASC")
            ->get_where("article_categories", array("parent_id" => CATEGORY_MEDIA, "enabled" => 1));
        	
        // Load available stats
        $this->data["states"] = $this->db->order_by("name", "ASC")
            ->get_where("states", array("country_id" => 1)); 
            
        // Load available areas
        $this->data["areas"] = $this->db->order_by("area_name", "ASC")
            ->get_where("areas", array("enabled" => 1));                       

        $this->load->view('member/header', $this->data);
        $this->load->view('member/media/list/prebody.php', $this->data); 
        $this->load->view('member/media/list/main.php', $this->data);
    ...
    function ajax()
    {
        ...
        switch($action)
        {
            case "load_media":   // User is trying to login.
                $this->handle_load_media();
                break;
                
            case "download_media":   // User is trying to login.
                $this->handle_download_media();
                break;
                
            case "email_resource":   // User is trying to login.
    ...
    */
    private function handle_download_media()
    {
        ...
        if($article->attachment != "")
        {
            ...
            $article->link_www = base_url() . "article_files/" . $article->article_id . "/" . $article->attachment;   
        } 
        ...
        elseif ($this->user_type_id == USER_TYPE_LEAD)
        {
        	$user_type = array(USER_TYPE_ADVISOR, USER_TYPE_INVESTOR, USER_TYPE_PARTNER, USER_TYPE_LEAD);
        }
        else 
        ...
        $users = $this->users_model->get_list(1, '', '', $count_all, "", $user_type, $filters, $assign_client_select_sql);

        $this->data["status"] = "OK";
        $this->data["message"] = $this->load->view("member/media/list/modal", array("article" => $article, "users" => $users, 'email_logged' => $user_logged->email), true);
    ...
    */
    private function handle_load_media()
    {
        ...
        $this->form_validation->set_rules('category_id', 'Category ID', 'required|number');
        ...
        $media = $this->article_model->get_media($filters, $order_by, MEDIA_PER_PAGE, $current_page, $count_all);

        $this->data["status"] = "OK";
        $this->data["message"] = $this->load->view("member/media/list/list", array("media" => $media), true);
