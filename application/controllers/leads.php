...
class Leads extends MY_Controller 
{
    ...
    function index()
    {
        $this->data["meta_title"] = "My Enquiries";
        
        $this->load->view('member/header', $this->data);
        $this->load->view('member/leads/list/prebody.php', $this->data); 
        $this->load->view('member/leads/list/main.php', $this->data);
    ...
    function detail($user_id = "")
    {
        ...
        $logged_in_user = $this->Users_model->get_details($this->user_id);
        if(!$logged_in_user) show_error("Couldn't load the currently logged in user object");
        ...
        $user_type = array(USER_TYPE_PARTNER);
        ...
        if($add_mode)
        {
            $this->data["meta_title"] = "Add New Enquiry";    
        }
        else
        {
            ...
            // If the user could not be loaded, OR if the user was created by someone else and is not owned by the logged in user,
            // forbid this action.
            ...
            $this->data["meta_title"] = "Enquiry Detail";
        }
        ...
        $this->load->view('member/header', $this->data);
        $this->load->view('member/leads/detail/prebody.php', $this->data); 
        $this->load->view('member/leads/detail/main.php', $this->data);
        $this->load->view('member/footer', $this->data); 
    }    
    ...
    function ajax()
    {
        ...
        switch($action)
        {
            case "update_lead":   // User is trying to login.
                $this->handle_update_lead();
                break;
                
            case "delete_lead":   // User is trying to login.
                $this->handle_delete_lead();
                break;
			
            case "update_to_investor":  // User is trying to login.
            	$this->handle_update_to_investor();
            	break;
            ...
            case "remove_login_privileges":
                $this->handle_remove_login_privileges();
                break;
    ...
    */
    private function handle_load_tasks()
    {
        ...
        $this->data["status"] = "OK";
        $this->data["message"] = $this->load->view("member/leads/detail/task_listing", array("tasks" => $tasks), true);
    ...
    */
    private function handle_load_notes()
    {
        ...
        $this->data["status"] = "OK";
        $this->data["message"] = $this->load->view("member/leads/detail/note_listing", array("notes" => $notes), true);
        $this->data["count_all"] = $count_all;
    ...
    */
    private function handle_load_leads()
    {
        ...
        //$extra_sql = ", get_lead_status_count(u.user_id, 'sold') as num_sold, get_lead_last_status_date(u.user_id, 'sold') as last_sold_date ";
        ...
        if($sort_col == "notes_last_created") $sort_col = "get_last_note_date(u.user_id)";
        ...
        $this->data["status"] = "OK";
        $this->data["message"] = $this->load->view("member/leads/list/list", array("users" => $users), true);
        $this->data["count_all"] = $count_all;
    ...
    */
    private function handle_update_lead()
    {
        // Load neccessary libs and models
        $this->load->library('form_validation');
        ...
        // Also, partners cannot update these fields.
        if(($user) && ($user->created_by_user_id == $this->user_id) && ($this->user_type_id != USER_TYPE_PARTNER))
        {
        ...
        if($add_mode)
        {
            ...
            $save["username"] = "LEAD_" . time() . "_" . $this->user_id;    
            $save["user_type_id"] = USER_TYPE_LEAD;
            $save["salt"] = random_string("alnum", 15);
            $save["created_by_user_id"] = $this->user_id;
            
            if($this->user_type_id == USER_TYPE_PARTNER) 
            {
                // If a partner is adding a new enquiry, automatically set the advisor and owner fields
                $logged_in_user = $this->Users_model->get_details($this->user_id);
                $save["advisor_id"] = $logged_in_user->created_by_user_id;
    ...
    */
    private function handle_delete_lead()
    {
        ...
        if($user_id)
        {
            ...
            {
            	$this->Users_model->save($user_id, array('deleted' => 1));
            }
        }
        $this->data["status"] = "OK";
        $this->data["message"] = "";
    ...
    */
    private function handle_update_to_investor()
    {
        ...
        if(is_numeric($user_id)) {
            ...
            if(stristr($user->username, "LEAD_")) {
                $this->form_validation->set_rules('email', 'Email Address', 'required|valid_email|callback__check_email');    
            } else {
        ...
        if ($this->form_validation->run() == FALSE)
        {
            //$this->data["message"] = "In order to upgrade this lead, please enter the lead's email address.";
            $this->data["message"] = validation_errors('- ', '');
            send($this->data);
        }
        ...
        if($user_id)
        {
			...
			$user_data['user_type_id'] = USER_TYPE_INVESTOR;
			$user_data["salt"] = random_string("alnum", 15);
            $password = random_string("alnum", 8);
            $user_data["password"] = hash("SHA256", $password . $user_data["salt"]);
            
            $user_id = $this->Users_model->save($user_id, $user_data);
            
            if(!$user_id)
	        {
	            $this->data["message"] = "Sorry, someone went wrong whilst trying to save this lead.";
	            send($this->data);             
	        }
	        else
	        {
	            ...
	            $this->email_model->send_email($user->email, "welcome_to_aspire", $email_data,  $attach = "", $bcc = array());    	
	        }
        }
    ...
    /***
    * Handles the lead_can_login action - when an advisor or partner flags that a lead is allowed to login to the system.
    */
    private function handle_lead_can_login()
    {
        // Load neccessary libs and models
        $this->load->library('form_validation');
        ...
        // Validate the form submission
        $this->form_validation->set_rules('user_id', 'User ID', 'required|number'); 
        ...
        $lead_id = $this->input->post("user_id");
        
        // Load the lead record
        $lead = $this->users_model->get_details($lead_id);
        if(!$lead)
        {
            $this->data["message"] = "Sorry, the lead id is invalid.  Please check your login session is still valid and if the error still persists, report this error.";
            send($this->data);            
        }
        
        // Make sure the user whom is trying to allow the lead to login has permission to  do so
        if((($lead->created_by_user_id != $this->user_id) && ($lead->owner_id != $this->user_id) && ($lead->owner_created_by_user_id != $this->user_id)))
        {
            $this->data["message"] = "Sorry, you do not have permission to perform this action.";
            send($this->data);                 
        }
        
        // Make sure the leads email address is set, as this will become
        if(($lead->email == "") || (!valid_email($lead->email)))
        {
            $this->data["message"] = "Please update this leads email address before proceeding.";
            send($this->data);                 
        }
        
        // See if another user with the same email address as the username exists in the database
        $exists = $this->users_model->get_details($lead->email, true);
        if($exists)
        {
            $this->data["message"] = "Sorry, another user in the database has this email address as their username.  Username's must be unique.  Please enter a different email address before trying to allow this lead to login.";
            send($this->data);              
        }
        
        // Everything is OK.  Update this user so they  can login
        $user_data = array();
        $user_data['username'] = $lead->email;
        $user_data["salt"] = random_string("alnum", 15);
        $password = random_string("alnum", 8);
        $user_data["password"] = hash("SHA256", $password . $user_data["salt"]);
        
        $lead_id = $this->Users_model->save($lead_id, $user_data);
        
        if(!$lead_id)
        {
            $this->data["message"] = "Sorry, someone went wrong whilst trying to update the lead with login credentials.";
            send($this->data);             
        }
        else
        {
            // Send the welcome email
            $email_data = array();
            $email_data["first_name"] = $lead->first_name;
            $email_data["email"] = $lead->email;
            $email_data["password"] = $password;
            $email_data["added_by"] = $this->session->userdata["first_name"] . " " . $this->session->userdata["last_name"];
            $email_data["login_link"] = base_url() . "login";
            
            $this->email_model->send_email($lead->email, "welcome_to_aspire", $email_data,  $attach = "", $bcc = array());        
        }        
    ...
    */
    private function handle_load_stock_project_permissions()
    {
        ...
        if($user_id)
        {
            ...
            $this->data["status"] = "OK";
            $this->data["message"] = $this->load->view("member/leads/detail/stock_project_permissions", $data, true);
            send($this->data);
        }
        else
        {
    ...
    private function handle_load_property_permissions()
    {
        ...
        if($user_id)
        {
			...
			$data = array(
                            ...
                            'logged_user_type' => $this->user_type_id
						);
					
            $this->data["status"] = "OK";
            $this->data["message"] = $this->load->view("member/leads/detail/stock_property_permissions", $data, true);
            send($this->data);
    ...
    */
    ...
    /***
    * Handles the remove_login_privileges action
    * Send an OK status back if remove login privileges successfully, error if not.
    */
    private function handle_remove_login_privileges()
    {
        // Load neccessary libs and models
        $this->load->library('form_validation');
        ...
        // Make sure the user whom is trying to allow the lead to login has permission to  do so
        if(!has_permission($lead, $this->user_id))
        {
            $this->data["message"] = "Sorry, you do not have permission to perform this action.";
            send($this->data);                 
        }
        ...
        // Everything is OK.  Update this user so they  can login
        $user_data = array();
        $user_data['username'] = "LEAD_" . time() . "_" . $this->user_id;
        $user_data["salt"] = '';
        $user_data["password"] = '';
    ...
    {
    	$exists = $this->Users_model->exists($email);
    	if ($exists)
    	{
    		$this->form_validation->set_message('_check_email', '%s is already used by another user.');
        	return false;
    	}
    	else
    	{
